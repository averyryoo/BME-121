        ...
        static void CycleSort( int[ ] a )
        {
            ...
            for( int cycleStart = 0; cycleStart < a.Length; cycleStart ++ )
            {
                ...
                while( ! cycleDone )
                {
                    if( pos == cycleStart )
                    {
						if(a[cycleStart] != item)
						{
							writes ++;
							a[cycleStart] = item;
						}
						
						// end of cycle so increment cycleStart
						cycleDone = true;
                    }
                    else /* ( pos != cycleStart ) */
                    {
						if(item != a[pos])
						{
							...
							// assign a temporary placeholder variable for what's in pos
							int temp = a[pos];
							
							// write item into the correct position
							writes  ++;
							a[pos] = item;
							
							// declare new item as the displaced value (temp)
							reads ++;
							item = temp;
							pos = Rank(item, skip: cycleStart);
						}
						
						// consider duplicate values that aren't at the 
						// start of the cycle and increment past them
